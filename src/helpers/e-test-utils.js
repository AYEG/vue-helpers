...
export function extendWrapper (wrapper) {
   ...
   * @returns {String}
   */
  wrapper.getTextFromInput = name => wrapper.getInput(name).element.value
   ...
   * @returns {Number}
   */
  wrapper.getIntFromInput = name => parseInt(wrapper.getInput(name).element.value, 10)
  ...
  wrapper.validateInputs = (validatorCallbackName, expect, obj) => {
    for (var i in obj) {
      expect(wrapper[validatorCallbackName](i)).toEqual(obj[i])
    }
  }
